...
* `useHostCookiePrefix`: Emit cookies with the `__Host-` prefix instead of `__Secure-`, whenever cookies are otherwise secure. Cannot be combined with `crossSubDomainCookies.enabled`, `useSecureCookies: false`, or a cookie that overrides `domain`/`path` — these throw at startup. Renames every auth cookie, signing out existing sessions.
